...
public class StatusEffect {
	...
	double[] resistanceModifiers = new double[types.length];
	float maxDuration;
	...
	public StatusEffect(GameElement owner, StackingProperty stackingProperty, String id, float duration, int level) {
		...
		this.remove = false;
		this.onCreate();
		this.damageType = DamageType.ABSOLUTE; //in case damage type never gets set
		this.numSources = 1;
		if(duration == -1) {
	...
	public StatusEffect clone(){
		...
		effect.resistanceModifiers = this.resistanceModifiers;
	...
	public void timingUpdate(float frametime) { //updates things related to time
		...
		if(this.damagetimer <= 0 && this.remove == false) { //if it's time to deal damage and it's not flagged for removal
			if(!this.muteEffect) {
				getOwner().doDamage(damagePerInterval, this.damageType);
				this.onInterval();
			}
	...
	public void setResistanceModifier(DamageType type, double resistance) {
		for(int i = 0; i < types.length; i++) { //goes through until it finds the damagetype's index
			if(type.equals(types[i])) { //if they're the same type
				if(resistance > 100) {
					this.resistanceModifiers[i] = 100;
				} else {
					this.resistanceModifiers[i] = resistance;
				}
				if(getOwner() != null) {
					this.getOwner().statusFinalModifierValueUpdate = true;
				}
	...
	public void changeResistanceModifier(DamageType type, double resistance) {
		for(int i = 0; i < types.length; i++) { //goes through until it finds the damagetype's index
			if(type.equals(types[i])) { //if they're the same type
				if(resistance + this.resistanceModifiers[i] > 100) {
					this.resistanceModifiers[i] = 100;
				} else {
					this.resistanceModifiers[i] += resistance;
				}
	...
	public void setResistanceModifiers(double[] resistances) {
		if(resistances.length != this.resistanceModifiers.length) {
			System.out.println("ERROR: Array mismatch in setBaseResistances function!"); //TODO: Remove debug
			return;
		}
		for(int i = 0; i < types.length; i++) {
			if(resistances[i] + this.resistanceModifiers[i] > 100) {
				this.resistanceModifiers[i] = 100;
			} else {
				this.resistanceModifiers[i] = resistances[i];
			}
		}
		if(getOwner() != null) {
			this.getOwner().statusFinalModifierValueUpdate = true;
	...
	public double getResistanceModifier(DamageType type) {
		for(int i = 0; i < types.length; i++) { //goes through until it finds the damagetype's index
			if(type.equals(types[i])) { //if they're the same type
				return this.resistanceModifiers[i];
			}
		}
		System.out.println("ERROR: No resistance type matched in the getResistance function!"); //TODO: Remove debug
		return 0;
	}
	public double[] getResistanceModifiers() {
		return this.resistanceModifiers;
	}
